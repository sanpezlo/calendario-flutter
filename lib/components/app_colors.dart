import 'package:flutter/material.dart';

// 1B1B1D

class AppColor {
  static Color primary = const Color(0XFF0D4688);
  static Color secondary = const Color(0XFFBE1622);
  static Color background = const Color(0XFFFFFFFF);
  static Color alternative = const Color(0XFFF5F5F5);
  static Color text = const Color(0XFF000000);
  static Color grey = const Color(0XFF9E9E9E);

  static Color transparent = const Color(0X00000000);

  static Color black = const Color(0XFF000000);
  static Color white = const Color(0XFFFFFFFF);
  // static Color kPrimary = const Color(0XFFBE1622);
  static Color kPrimary = const Color(0XFF0D4688);
  static Color kSecondary = const Color(0XFFBE1622);
  static Color kBackGroundColor = const Color(0XFF2D3047);
  // static Color kLightAccentColor = const Color(0XFFF4E5F7);
  static Color kLightAccentColor = const Color(0XFF0D4688);
  static Color kGreyColor = const Color(0XFF939999);
  static Color kSamiDarkColor = const Color(0XFF313333);
  static Color kBlackColor = const Color(0XFF000000);
  // static Color kGrey3Color = const Color(0XFF272828);
  static Color kGrey3Color = const Color(0XFFF5F5F5);
  // static Color kBGColor = const Color(0XFF181A1A);
  static Color kBGColor = const Color(0XFFFFFFFF);
}
